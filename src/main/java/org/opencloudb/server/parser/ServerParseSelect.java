...
public final class ServerParseSelect {
	...
	/**
	 *  判断是否是 SELECT VERSION
	 * @param stmt 执行语句
	 * @param offset 开始位置
	 * @return 含有返回对应的id 否则返回other对应的值
	 */
	...
	/**
	 *  stmt.charAt(offset) == first 'L' OR 'l'
	 * @param stmt 执行语句
	 * @param offset 开始位置
	 * @return index after LAST_INSERT_ID(), might equals to length. -1 if not
	 *         LAST_INSERT_ID
	 ...
	 * @param stmt 执行语句
	 * @param offset 开始位置
	 * @returnindex after identity or `identity` or "identity" or 'identity',
